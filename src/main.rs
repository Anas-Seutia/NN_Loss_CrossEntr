...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let root_area = BitMapBackend::new("loss_chart.png", (640, 480)).into_drawing_area();
    root_area.fill(&WHITE)?;

    let mut chart = ChartBuilder::on(&root_area)
        .caption("Loss over Epochs", ("sans-serif", 40).into_font())
        .margin(10)
        .x_label_area_size(30)
        .y_label_area_size(30)
        .build_cartesian_2d(0..1000, 0f64..10f64)?;
    ...
    for epoch in 0..epochs {
        ...
        for (input, &output) in inputs.iter().zip(outputs.iter()) {
            ...
            // loss
            total_loss += cross_entropy_loss(output, prediction);
            
            // Backward pass (Gradient Descent)
            let error = prediction - output;
            for (j, input_val) in input.iter().enumerate() {
                weights[j] -= learning_rate * error * input_val;
            }
            bias -= learning_rate * error;
        }
        epoch_losses.push((epoch, total_loss));
    }
...
fn cross_entropy_loss(y_true: f64, y_pred: f64) -> f64 {
